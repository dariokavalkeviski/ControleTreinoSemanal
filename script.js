...
// ==================== EXPORTAÇÃO FICHA DE TREINO ====================

function exportarFichaTreino() {
  // Tenta pegar os dados do último resultado exibido
  let nome = "—";
  let peso = "—";
  let altura = "—";
  let imcValue = "—";
  ...
  // Se o resultado do IMC estiver visível, pega os dados do histórico
  const historico = obterHistorico();
  if (historico.length > 0) {
    const ultimaMedicao = historico[historico.length - 1];
    nome = ultimaMedicao.nome || "—";
    peso = ultimaMedicao.peso || "—";
    altura = ultimaMedicao.altura || "—";
    imcValue = ultimaMedicao.imc ? ultimaMedicao.imc.toFixed(1) : "—";
    imcClass = ultimaMedicao.classificacao || "—";
  } else {
    // Se não houver histórico, pega dos campos do formulário
    nome = document.getElementById("nomeUsuario").value || "—";
    peso = document.getElementById("pesoUsuario").value || "—";
    altura = document.getElementById("alturaUsuario").value || "—";
    imcValue = document.getElementById("imcValue")?.textContent || "—";
    imcClass = document.getElementById("imcClassification")?.textContent || "—";
  }

  // Coleta os treinos
  const treinosHTML = document.getElementById("treinosContainer").innerHTML;
  ...
  const fichaHTML = `
    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: auto;">
      ...
      <table style="width:100%; border-collapse: collapse; margin-bottom: 20px;">
        <tr>
          <th style="border:1px solid #aaa; padding:6px; text-align:center;">Nome</th>
          <th style="border:1px solid #aaa; padding:6px; text-align:center;">Peso (kg)</th>
          <th style="border:1px solid #aaa; padding:6px; text-align:center;">Altura (m)</th>
          <th style="border:1px solid #aaa; padding:6px; text-align:center;">IMC</th>
        </tr>
        <tr>
          <td style="border:1px solid #aaa; padding:6px; text-align:center;">${nome}</td>
          <td style="border:1px solid #aaa; padding:6px; text-align:center;">${peso}</td>
          <td style="border:1px solid #aaa; padding:6px; text-align:center;">${altura}</td>
          <td style="border:1px solid #aaa; padding:6px; text-align:center;">${imcValue} (${imcClass})</td>
        </tr>
      </table>
      ${treinosHTML}
    </div>
    <style>
      ...
      th { background: #eee; }
      button, .btnConcluir { display: none !important; }
      hr { display: none; }
    </style>
  `;

  // Abre janela para impressão
  const win = window.open("", "", "width=900,height=700");
  win.document.write("<html><head><title>Ficha de Treino</title></head><body>");
...
// Adiciona botão na página
window.addEventListener("DOMContentLoaded", () => {
  const btn = document.createElement("button");
  btn.textContent = "Exportar/Imprimir Ficha de Treino";
  btn.style.margin = "20px auto 30px auto";
  btn.style.display = "block";
  btn.onclick = exportarFichaTreino;
  document.body.insertBefore(btn, document.getElementById("treinosContainer"));

  // Removido o título "Treinos" conforme solicitado
  // const tituloTreinos = document.createElement("h3");
  // tituloTreinos.textContent = "Treinos";
  // container.appendChild(tituloTreinos);
});
